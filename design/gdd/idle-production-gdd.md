...
> **Depends On**:
> - **#1 Data Persistence & Roster Core** (`persistence-gdd.md` v1.2) — OWNS storage, the atomic mutation path, `lastSeen`, `pendingOffline`, the `roster` map, and the **`base` (BaseData)** + `pendingOnline` fields (added to the v1 baseline in persistence v1.2, reconciled with this GDD). Idle does NOT touch DataStore directly.
...
> - **#9 Economy** (`economy-gdd.md` v1.1) — OWNS the wallet + cost curves; Idle deposits via `Economy.award(player, amount, "idle_collect")` and spends via `Economy.spend(...)` for upgrades.

> **Depended On By**: Raid v1 (#6, loot = % of uncollected pending), Work-Based Evolution (#8, production milestones via `history.coins`), Moment System (#12, listens to Idle-emitted events), Daily Quests (#17, "collect X coins" / progress events).
...
1. **Per-Brainrot base rate** — every deployed Brainrot produces `baseRatePerWorker` coins/sec (config, §8). Species/level/evolution may scale it later (forward-compat hook `levelRateBonus`, default 0 at MVP so launch is flat per worker).
...
1. **Factory view** — the player's building with its **worker slots** laid out as a simple row/grid of "desks." Empty slots show a "+" (tap to deploy from roster); occupied slots show the deployed Brainrot with its personality badge (#2) and state icon (Working = none, OnBreak = "zzz/coffee", Walkout = tiny protest sign).
...
|---|---|---|
| **#1 Data Persistence** | both | Reads `roster`, `lastSeen`, `pendingOffline`, and the **`base` (BaseData)** + `pendingOnline` (✅ added to the v1 baseline in persistence v1.2, §3 flag resolved). Supplies `IdleProduction.computeOfflineRate(roster, base)` for persistence `F-OFFLINE`. Writes go through Persistence's atomic path (Collect, deploy, upgrade-level bumps, `history.coins`). Never touches DataStore directly. |
