...
> 🔄 **REBRAND NOTICE (2026-06-07)**: Tema diganti per `design/decisions/2026-06-07-monster-relic-rebrand.md` — kreatur "Brainrot" → **Monster Relic**. Boss tiers dapat identitas lore baru: Tier 1 Colony Boss = **penjaga gerbang**, Tier 2 "CEO Brainrot" → kandidat **Temple Guardian** (penjaga kuil), Tier 3 Raid Boss = **penjaga harta** di Ancient Vault.

> ⚡ **ELEMENT SYSTEM LOCKED (2026-06-07)**: open decision Element **resolved** — ekstensi 3-tier TIDAK terblokir lagi. Formula damage dokumen ini mendapat faktor baru: `dmg = baseDmg * levelScale(L) * elementMod(attacker.element, defender.element)` dengan lingkaran **Fire > Nature > Water > Volt > Wind > Fire**, mod 1.25/1.00/0.75 (config), berlaku dua arah (summon→boss dan boss→summon), elemen boss fixed per spec + ditampilkan pre-encounter. Integrasikan ke §8 saat revisi ekstensi 3-tier ditulis. Lihat `design/decisions/2026-06-07-element-system.md`.
